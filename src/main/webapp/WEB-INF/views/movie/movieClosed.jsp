...
<link href="css/movieClosed.css?update" rel="stylesheet" type="text/css">
<style type="text/css">
	@font-face
    {
...
<c:if test="${startPage>PAGE_PER_BLOK}">
 	<button class="btn btn-outline-dark btn-sm" onclick="location.href='movieClosed.mv?pageNum=${startPage-1}'">이전</button>
</c:if>
 <c:forEach var="i" begin="${startPage }" end="${endPage }">
<c:if test="${currentPage==i }">
 <button class="btn btn-outline-dark btn-sm" onclick="location.href='movieClosed.mv?pageNum=${i}'" disabled="disabled">${i }</button> 
</c:if>
	
 <c:if test="${currentPage!=i }">
 <button class="btn btn-outline-dark btn-sm" onclick="location.href='movieClosed.mv?pageNum=${i}'">${i }</button> 
</c:if>
 </c:forEach>
<c:if test="${currentPage < totalPage}">
						<button class="btn btn-outline-dark btn-sm" onclick="location.href='movieClosed.mv?pageNum=${currentPage + 1}'">다음</button>
					</c:if>
<c:if test="${endPage<totalPage }">
 
 		<button class="btn btn-outline-dark btn-sm" onclick="location.href='movieClosed.mv?pageNum=${endPage+1}'">다음</button>
</c:if>
</div>
</div>
</div>
